...
class KotacomAI_API_Handler {
    ...
    public function generate_content($prompt, $parameters = array()) {
        ...
        // Attempt to call API with retry logic for rate limits
        $max_retries = 3;
        $attempt = 0;
        ...
        while ($attempt < $max_retries) {
            ...
            switch ($provider) {
                case 'google_ai':
                    $result = $this->call_google_ai($final_prompt, $api_key, $parameters);
                    break;
                case 'openai':
                    $result = $this->call_openai($final_prompt, $api_key, $parameters);
                    break;
                case 'groq':
                    $result = $this->call_groq($final_prompt, $api_key, $parameters);
                    break;
                case 'cohere':
                    $result = $this->call_cohere($final_prompt, $api_key, $parameters);
                    break;
                case 'huggingface':
                    $result = $this->call_huggingface($final_prompt, $api_key, $parameters);
                    break;
                case 'together':
                    $result = $this->call_together($final_prompt, $api_key, $parameters);
                    break;
                case 'anthropic':
                    $result = $this->call_anthropic($final_prompt, $api_key, $parameters);
                    break;
                case 'replicate':
                    $result = $this->call_replicate($final_prompt, $api_key, $parameters);
                    break;
                case 'openrouter':
                    $result = $this->call_openrouter($final_prompt, $api_key, $parameters);
                    break;
                case 'perplexity':
                    $result = $this->call_perplexity($final_prompt, $api_key, $parameters);
                    break;
                default:
                    return array(
                        'success' => false,
        ...
        if (isset($rotation_result) && $rotation_result['rotated']) {
            $result['error'] .= ' (Tried rotating API keys, but all keys may be rate limited)';
        }
