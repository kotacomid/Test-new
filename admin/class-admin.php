...
class KotacomAI_Admin {
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            'kotacom-ai',
            __('API Key Rotator', 'kotacom-ai'),
            __('API Rotator', 'kotacom-ai'),
            'manage_options',
            'kotacom-ai-api-rotator',
    ...
    /**
     * Display API rotator page
     */
    public function display_api_rotator_page() {
        // Add help information
        $this->add_api_rotator_help_info();
        
        include KOTACOM_AI_PLUGIN_DIR . 'admin/views/api-rotator.php';
    }

    /**
     * Add help information for API rotator page
     */
    private function add_api_rotator_help_info() {
        add_action('admin_notices', function() {
            $this->display_help_box(
                __('API Key Rotator', 'kotacom-ai'),
                '<p>' . __('Manage multiple API keys per provider to automatically handle rate limits and ensure continuous operation.', 'kotacom-ai') . '</p>' .
                '<p><strong>' . __('How it works:', 'kotacom-ai') . '</strong> ' . __('When one API key hits a rate limit, the system automatically switches to the next available key.', 'kotacom-ai') . '</p>'
            );
            
            $tips = array(
                __('Add multiple API keys for each provider to maximize uptime', 'kotacom-ai'),
                __('Keys in cooldown will be automatically skipped until they recover', 'kotacom-ai'),
                __('Test all keys regularly to ensure they are working', 'kotacom-ai'),
                __('Monitor rotation statistics to identify problematic keys', 'kotacom-ai'),
                __('Free tier providers may have lower limits - consider multiple accounts', 'kotacom-ai')
            );
            $this->display_tips_box($tips);
        });
    }
